function [  ] = Draw_graph_weight( filename,n,optchangepoint,weight)
...
%%%%%%%%%%%%%%%%% to be the number of data points.
...
B=[];
for i=1:L2-1
    V=[V sum(weight(optchangepoint(i):(optchangepoint(i+1)-1)))];
    B=[B A(optchangepoint(i+1)-1)-A(optchangepoint(i))];
    rate=[rate V(i)./B(i)];
end 
display(rate); 
block=[];
